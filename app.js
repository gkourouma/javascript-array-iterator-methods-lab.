...
let sortedByBirthYear = inventors.sort((a,b) => a.year - b.year); // a and b represents two element that are being compared in the year property. If a - b = (+) number, 'b' will be placed ahead of 'a', if a - b = (-) number, 'a' will be placed ahead of "b". If the answer is 0 then no switching.
...
let inventorNamedAda = inventors.find((names) => {
...
let firstLast = people.map(name => {
    let [last, first] = name.split(', '); // splitting the string at the comma ',' and assinging the new values to last and first
    return `${first} ${last}` // rearraning it to print out first and last instead.
});
...
// Complete the exercise in the space below:
let totalYearsLived = inventors.reduce(function(accumulator, inventor) {
